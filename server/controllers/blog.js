    ...
    .exec(function(err, posts) {
      if (err) {
        ...
        return res.status(422).json({
          message: 'Terjadi kesalahan dalam website.'
        });
      }
      res.json(posts);
    });
...
exports.createPost = function(req, res, next) {
  ...
  if (!title || !categories || !content) {
    return res.status(422).json({
      message: 'Judul, kategori, dan konten harus diisi.'
    });
  }
...
exports.fetchPost = function(req, res, next) {
  Post.findById({
    ...
    if (err) {
      console.log(err);
      return res.status(422).json({
        message: 'Tidak ditemukan postingan dengan ID yang diberikan.'
      });
    }
    if (!post) {
...
exports.createComment = function(req, res, next) {
  ...
  if (!user) {
    return res.status(422).json({
      message: 'Anda harus masuk sebelum memberikan komentar.'
    });
  }
  ...
  if (!content) {
    return res.status(422).json({
      message: 'Komentar tidak bisa kosong.'
    });
  }
  ...
  Comment
    ...
    .exec(function(err, comments) {
      if (err) {
        console.log(err);
        return res.status(422).json({
          message: 'Terjadi kesalahan pada website.'
        });
      }
      res.json(comments);
    });
