...
exports.signup = function(req, res, next) {
  ...
  if (!email || !password) {
    return res.status(422).send({ message: 'Bagian email dan password wajib diisi.' });  // 422 refers to unprocessable entity
  }
  ...
  User.findOne({ email: email }, function(err, existingUser) {
    ...
    if (existingUser) {
      return res.status(422).send({ message: 'Email sudah digunakan.' });  // 422 refers to unprocessable entity
    }
    ...
    user.save(function(err) {  // callback function
      ...
      res.json({ message: 'Akun Anda sudah terdaftar. Anda bisa masuk ke dalam sistem sekarang.' });
    });
  });
};
