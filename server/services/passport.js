...
const localLogin = new LocalStrategy(localOptions, function(email, password, done) {
  ...
  User.findOne({ email: email }, function(err, user) {
    ...
    if (!user) {
      return done(null, false, { message: 'Username salah.' });  // that last argument is the info argument to the authenticate callback
    }
    ...
    user.comparePassword(password, function(err, isMatch) {
      ...
      if (!isMatch) {
        return done(null, false, { message: 'Password salah.' });  // that last argument is the info argument to the authenticate callback
      }
