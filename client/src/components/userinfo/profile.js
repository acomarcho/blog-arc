    ...
    if (state && action === 'REPLACE') {
      ...
      return (
        <div className={`alert ${state.status === 'success' ? 'alert-success' : 'alert-danger'}`} role="alert">
          {`[${state.time}] --- `} <strong>{state.status === 'success' ? 'Berhasil!' : 'Oops!'}</strong> {state.message}
        </div>
      );
    }
  }
    ...
    return (
      <div>
        ...
        <form className="form-profile" onSubmit={handleSubmit(this.handleFormSubmit.bind(this))}>
          ...
          <Field name="firstName" component={this.renderInput} type="text" label="Nama Awal:" required={true} />
          <Field name="lastName" component={this.renderInput} type="text" label="Nama Akhir:" required={true} />
          <Field name="birthday" component={this.renderInput} type="date" label="Tanggal Lahir:" />
          <Field name="sex" component={this.renderOptions} label="Jenis Kelamin:" />
          <Field name="phone" component={this.renderInput} type="text" label="Nomor HP:" />
          <Field name="address" component={this.renderInput} type="text" label="Alamat:" />
          <Field name="occupation" component={this.renderInput} type="text" label="Pekerjaan:" />
          <Field name="description" component={this.renderTextarea} label="Deskripsi Singkat:" />
          <button action="submit" className="btn btn-primary">Perbarui Profil</button>
        </form>
      </div>
    );
  }
