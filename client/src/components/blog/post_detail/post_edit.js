...
class PostEdit extends Component {
  ...
  render() {
    ...
    return (
      <div className="post">
        {this.renderAlert()}
        <h2 className="mb-5">Ubah Postingan Anda</h2>
        <form onSubmit={handleSubmit(this.handleFormSubmit.bind(this))}>
          <Field name="title" component={this.renderInput} type="text" label="Judul:" placeholder="Masukkan judul postingan" required={true} />
          <Field name="categories" component={this.renderInput} type="text" label="Categories:" placeholder="Masukkan kategori, pisahkan kategori-kategori dengan tanda koma (,)" required={true} />
          <Field name="content" component={this.renderTextEditor} label="Konten:" />
          <button action="submit" className="btn btn-primary">Publikasikan</button>
        </form>
      </div>
    );
  }
