    ...
    if (state && action === 'REPLACE') {
      return (
        <div className="alert alert-danger">
          {`[${state.time}] --- `} <strong>Gagal.</strong> {state.message}
        </div>
      );
    }
  }
    ...
    return (
      <div>
        ...
        <form className="form-signin" onSubmit={handleSubmit(this.handleFormSubmit.bind(this))}>
          <h3>Buat Akun</h3>
          <hr />
          <Field name="firstName" component={this.renderField} type="text" label="Nama Awal"/>
          <Field name="lastName" component={this.renderField} type="text" label="Nama Akhir"/>
          <Field name="email" component={this.renderField} type="email" label="Email"/>
          <Field name="password" component={this.renderField} type="password" label="Password"/>
          <Field name="passwordConfirm" component={this.renderField} type="password" label="Konfirmasi Password"/>
...
function validate(formProps) {
  ...
  */
  ...
  if (formProps.password !== formProps.passwordConfirm) {
    errors.passwordConfirm = 'Password harus sama!';
  }
